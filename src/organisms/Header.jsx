    ...
    <>
      <header className="primera" id="mainHeader">
        ...
        <nav className={`opciones ${isMenuOpen ? 'open' : ''}`}>
          ...
          {user && user.usuario && user.usuario.rol && (user.usuario.rol.nombre === 'ADMIN' || user.usuario.rol.nombre === 'EMPLEADO' || user.usuario.rol.nombre === 'USER') && (
        ...
        <div className={`botones-auth ${isMenuOpen ? 'open' : ''}`}>
          {user ? (
            ...
            <>
              <Link to="/login" onClick={toggleMenu} className="boton-inicio login-button">Iniciar Sesión</Link>            </>
          )}
        </div>
      </header>
    </>
